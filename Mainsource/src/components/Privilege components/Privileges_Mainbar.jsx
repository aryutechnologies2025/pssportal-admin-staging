...
const Privileges_Mainbar = () => {
  const [privilegeFor, setPrivilegeFor] = useState("role");
  const [selectedRole, setSelectedRole] = useState(null);
  const [selectedEmployee, setSelectedEmployee] = useState(null);
  ...
  const fetchPrivileges = async () => {
    try {
      ...
      setRoleOption(response?.data?.roles.map((role) => ({ 
  ...
  const validateAddForm = () => {
    ...
    // Check if role/employee is selected
    if (privilegeFor === "role" && !selectedRole) {
      errors.role = "Please select a role";
    } else if (privilegeFor === "employee" && !selectedEmployee) {
    ...
    Object.values(permissions).forEach(permission => {
      if (permission && Object.values(permission).some(val => val === 1)) {
        hasPermission = true;
      }
    });
  ...
  const validateEditForm = () => {
    ...
    // Check if at least one permission is selected
    let hasPermission = false;
    Object.values(permissionsedit).forEach(permission => {
      if (permission && Object.values(permission).some(val => val === 1)) {
        hasPermission = true;
      }
    });
      ...
      {loading ? (
        ...
        <>
          <div>
                  ...
                  <div className="flex md:flex-row flex-wrap items-center gap-5">
                    <div className="relative w-64">
                      ...
                      <InputText
                        ...
                        onChange={(e) => setGlobalFilter(e.target.value)}
                        placeholder="Search......"
                        className="w-full pl-10 pr-3 py-2 rounded-md border border-[#D9D9D9] focus:outline-none focus:ring-2 focus:ring-[#1ea600]"
                      />
                    </div>
                  </div>
                  <div className="hidden md:block">
            ...
            {addPrivilegesModalOpen && (
              <div className="fixed inset-0 bg-black/10 backdrop-blur-sm bg-opacity-50 z-50">
                <div className="absolute inset-0" onClick={closePrivilegesModal}></div>
                <div className={`fixed top-0 right-0 overflow-y-auto w-full h-full md:w-[53vw] bg-white shadow-lg transform transition-transform duration-500 ease-in-out ${isAnimating ? "translate-x-0" : "translate-x-full"}`}>
                  <div className="w-6 h-6 rounded-full mt-2 ms-2 border-2 transition-all duration-500 bg-white border-gray-300 flex items-center justify-center cursor-pointer" onClick={closePrivilegesModal}>
                    <IoIosArrowForward className="w-3 h-3" />
                  </div>
                  ...
                  <div className="px-5 lg:px-14 py-3 md:py-10">
                    ...
                    {/* ROLE / EMPLOYEE DROPDOWN */}
                    <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] items-center gap-4 mb-2 md:mb-4">
                      <label className="text-md font-medium">
                        {privilegeFor === "role" ? "Role" : "Employee"}
                      </label>
                      <div>
                          ...
                          <Dropdown
                            ...
                            onChange={(e) => {
                              ...
                              if (formErrors.employee) {
                                setFormErrors(prev => ({...prev, employee: null}));
                              }
                            }}
                            options={employeeOption}
                            placeholder="Select Employee"
                        ...
                        {formErrors.role && (
                          <div className="text-red-500 text-sm mt-1">{formErrors.role}</div>
                        )}
                        {formErrors.employee && (
                          <div className="text-red-500 text-sm mt-1">{formErrors.employee}</div>
                        )}
                      </div>
                    </div>

                    {/* Permissions Section */}
                    <div className="mt-3 md:mt-10 rounded-lg">
                      <div className="space-y-8">
                        {privilegesOption.map((section) => (
                          <div key={section.name} className="border rounded-lg p-4 bg-white shadow">
                              ...
                              <div key={child.module} className="mb-5">
                                ...
                                <div className="flex flex-wrap gap-5">
                                  {child.actions.map((action) => {
                                    const moduleKey = child.module.toLowerCase();
                                    const permissionKey = actionMap[action];
                                    const checked = permissions?.[moduleKey]?.[permissionKey] === 1;

                                    return (
                                      <label key={`${child.module}-${action}`} className="flex items-center gap-2 text-sm">
                                        <input
                                          type="checkbox"
                                          checked={checked}
                                          onChange={(e) => {
                                            ...
                                            setPermissions((prev) => {
                                              ...
                                              let hasPermission = false;
                                              Object.values(newPerms).forEach(perm => {
                                                if (perm && Object.values(perm).some(val => val === 1)) {
                                                  hasPermission = true;
                                                }
                                              });
                                              
                                              if (hasPermission && formErrors.permissions) {
                      ...
                      {formErrors.permissions && (
                        <div className="text-red-500 text-sm mt-4 bg-red-50 p-3 rounded-md border border-red-200">
                          {formErrors.permissions}
                        </div>
                      )}

                      <div className="flex md:justify-end gap-2 mt-8">
                        <button onClick={closePrivilegesModal} className="hover:bg-[#FEE2E2] hover:border-[#FEE2E2] text-sm md:text-base border border-[#7C7C7C] text-[#7C7C7C] hover:text-[#DC2626] px-5 md:px-5 py-1 md:py-2 font-semibold rounded-[10px] transition-all duration-200">
                          Cancel
                        </button>
                        <button className="bg-[#1ea600] text-white hover:bg-green-600 px-4 md:px-5 py-2 font-semibold rounded-[10px] transition-all duration-200" onClick={handleSubmit}>
                          Submit
                        </button>
                      </div>
                    </div>
              ...
              <div className="fixed inset-0 bg-black/10 backdrop-blur-sm bg-opacity-50 z-50">
                ...
                <div className={`fixed top-0 right-0 overflow-y-auto w-full h-full md:w-[53vw] bg-white shadow-lg transform transition-transform duration-500 ease-in-out ${isAnimating ? "translate-x-0" : "translate-x-full"}`}>
                  ...
                  <div className="px-5 lg:px-14 py-3 md:py-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] items-center gap-4 mb-6 mt-5">
                      <label className="text-md font-medium">Privilege For</label>
                      <div className="flex gap-6">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input
                            type="radio"
                    ...
                    <div className="flex flex-wrap md:flex-nowrap gap-3 md:gap-14 items-center justify-between mb-6">
                      <label className="block text-md font-medium">
                        {privilegeForedit === "role" ? "Role" : "Employee"}
                      </label>
                      {privilegeForedit === "role" ? (
                    ...
                    <div className="mt-3 md:mt-10 rounded-lg">
                              ...
                              <div key={child.module} className="mb-5">
                                ...
                                <div className="flex flex-wrap gap-5">
                                  {child.actions.map((action) => {
                                    const moduleKey = child.module.toLowerCase();
                                    const permissionKey = actionMap[action];
                                    const checked = Number(permissionsedit?.[moduleKey]?.[permissionKey]) === 1;
                                        ...
                                        <input
                                          ...
                                          onChange={(e) => {
                                            ...
                                            setPermissionsedit((prev) => {
                                              ...
                                              if (hasPermission && editFormErrors.permissions) {
                                                setEditFormErrors(prev => ({...prev, permissions: null}));
                                              }
                      ...
                      {/* Edit permission validation error */}
                      {editFormErrors.permissions && (
                        <div className="text-red-500 text-sm mt-4 bg-red-50 p-3 rounded-md border border-red-200">
                          {editFormErrors.permissions}
                        </div>
                      )}
                      ...
                      <div className="flex md:justify-end gap-2 mt-8">
                        ...
                        <button className="bg-[#1ea600] text-white hover:bg-green-600 px-4 md:px-5 py-2 font-semibold rounded-[10px] transition-all duration-200" onClick={handleSubmitedit}>
                          Update
                        </button>
                      </div>
                    </div>
