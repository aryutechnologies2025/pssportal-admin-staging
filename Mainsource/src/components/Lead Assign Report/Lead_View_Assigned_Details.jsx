...
setFilters(data);
  ...
  const fetchAssignedLeads = async (appliedFilters = {}) => {
    ...
    try {
      ...
      if (
        ...
        appliedFilters.category_id.length > 0
      ) {
        params.category_id = appliedFilters.category_id.join(",");
      }

      if (
        Array.isArray(appliedFilters.lead_status) &&
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Platform",
      body: (row) =>
        Capitalise(row?.category?.name) || row.category_name || "-",
    },
    {
      field: "created_time",
      header: "Date",
...
<div className="w-full mt-6 bg-white rounded-2xl border border-gray-200 shadow-[0_10px_30px_rgba(0,0,0,0.06)] overflow-hidden">
  {/* Header */}
  <div className="flex items-start md:items-center justify-between gap-4 px-6 py-5 border-b border-gray-100">
    <div>
      <h2 className="text-lg md:text-xl font-bold text-gray-900">
        Lead Assigned Details
      </h2>
      <p className="text-sm text-gray-500 mt-1">
        View-only details (editing is disabled)
      </p>
    </div>

    <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-green-50 border border-green-200">
      <span className="h-2.5 w-2.5 rounded-full bg-[#1ea600]" />
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 md:col-span-2">
        ...
        <div className="flex flex-wrap gap-2">
          {(filters.lead_statuses || []).length > 0 ? (
            filters.lead_statuses.map((status, index) => (
              <span
                key={index}
                className="px-3 py-1.5 rounded-full bg-green-50 border border-green-200 text-green-800 text-xs font-semibold"
              >
                {status?.label || status}
              </span>
            ))
            ...
            <div className="flex flex-col w-full mt-1 md:mt-5 h-auto rounded-2xl bg-white shadow-[0_8px_24px_rgba(0,0,0,0.08)] px-2 py-2 md:px-6 md:py-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-semibold text-gray-700">Assign Leads</h3>
