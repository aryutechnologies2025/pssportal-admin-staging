...
const Lead_Dashboard = () => {
  ...
  const dummyLeadDetails = [
    ...
    { id: 3, name: "Bob Johnson", category: "instagram", status: "interested", date: new Date().toISOString(), email: "bob@example.com", phone: "1122334455" },
    { id: 4, name: "Alice Brown", category: "portal", status: "not_interested", date: new Date().toISOString(), email: "alice@example.com", phone: "5566778899" },
    { id: 5, name: "Charlie Wilson", category: "facebook", status: "follow_up", date: new Date().toISOString(), email: "charlie@example.com", phone: "6677889900" },
    { id: 6, name: "David Lee", category: "instagram", status: "open", date: new Date().toISOString(), email: "david@example.com", phone: "7788990011" },
    { id: 7, name: "Emma Davis", category: "portal", status: "joined", date: new Date().toISOString(), email: "emma@example.com", phone: "8899001122" }
  ];
  ...
  const generateStatusData = (category) => {
    ...
    const categoryLeads = dummyLeadDetails.filter(lead => lead.category === category);
    ...
    const statusDataArray = [
      ...
      { id: 3, status: "interested", count: statusCounts.interested, label: "Interested / Scheduled" },
      { id: 4, status: "not_interested", count: statusCounts.not_interested, label: "Not Interested" },
      { id: 5, status: "follow_up", count: statusCounts.follow_up, label: "Follow Up" },
      { id: 6, status: "not_picked", count: statusCounts.not_picked, label: "Not Picked" }
    ];
  ...
  const getFilteredLeads = () => {
    ...
    return dummyLeadDetails.filter(lead => 
...
/>
          ...
          {/* Category Modal */}
          <Dialog
            header={`${selectedCategory?.label} - Status Wise Count`}
          ...
          >
            <div className="p-4">
              <p className="text-sm text-gray-600 mb-4">
                Showing status counts 
                for <span className="font-semibold">{selectedCategory?.label}</span> 
              ...
              <div className="mt-4 p-3 bg-green-50 rounded-lg">
                <p className="text-sm text-green-800">
                  Total in {selectedCategory?.label}: {statusData.reduce((sum, item) => sum + item.count, 0)}
                </p>
              </div>
            </div>
          </Dialog>
          ...
          {/* Status Modal */}
          <Dialog
            header={`${selectedCategory?.label} - ${selectedStatus?.label} Leads`}
          ...
          >
            <div className="p-4">
              <p className="text-sm text-gray-600 mb-4">
                Showing leads from <span className="font-semibold">{selectedCategory?.label}</span> 
                with status <span className="font-semibold">{selectedStatus?.label}</span>
              </p>
              
              <DataTable
                value={getFilteredLeads()}
                showGridlines
                responsiveLayout="scroll"
                className="border rounded-lg"
                paginator
              ...
              <div className="mt-4 p-3 bg-green-50 rounded-lg">
                <p className="text-sm text-green-800">
                  Total Count: {getFilteredLeads().length}
                </p>
              </div>
            </div>
          </Dialog>
